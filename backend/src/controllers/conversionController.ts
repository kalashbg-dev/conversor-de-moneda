...
export const logConversion = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (institution_exchange_rate_id) {
      ...
      exchangeRate = await InstitutionExchangeRate.findById(institution_exchange_rate_id)
        .where('currencyFrom').equals(newCurrencyFrom)
        .where('currencyTo').equals(newCurrencyTo);
      ...
      if (!exchangeRate) {
        ...
        res.status(404).json({ message: 'Tasa de cambio de institución no encontrada' });
      ...
      if (!exchangeRate) {
        ...
        res.status(404).json({ message: 'Tasa de cambio estándar no encontrada' });
        return;  // Retornar aquí para evitar continuar la ejecución
      } else {
    ...
    const conversion = new Conversion({
      ...
      amount,
      result,
      exchange_rate_id: institution_exchange_rate_id ? undefined : exchangeRate._id,
      institution_exchange_rate_id: institution_exchange_rate_id ? exchangeRate._id : undefined,
    });
...
export const getConversionHistory = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!conversions || conversions.length === 0) {
      res.status(404).json({ message: 'No se encontró historial de conversiones' });
      return;
    }
    ...
    res.status(500).json({ message: 'Error al obtener el historial de conversiones', error });
  }
};
